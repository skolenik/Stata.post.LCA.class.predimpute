...
\author{Kolenikov}{%
  Stas Kolenikov\\NORC at the University of Chicago\\Columbia, Missouri, USA\\kolenikov-stas@norc.org
}
\title[Post-estimation for LCA via MI]{Inference for predicted latent classes using multiple imputation}
\maketitle
...
I introduce a new command \stcmd{postlca\_class\_predpute} to help with these tasks.
...
\stcmd{postlca\_class\_predpute}, addresses the issue of permitting 
...
\begin{enumerate}
    ...
    \item The multinomial regression model is fit to the classes as outcomes, with external predictors
          of class membership (as in, e.g. \stcmd{mlogit Chat x1 x2 x3})
\end{enumerate}
...
\section{The \stcmd{postlca\_class\_predpute} command}
\label{sec:new_command}

Following \stcmd{gsem, lclass()} estimation, users can obtain
multiply imputed latent classes using \stcmd{postlca\_class\_predpute} command
...
\stcmd{postlca\_class\_predpute} to facilitate analyses that
